...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Get profile error:', { error, cid })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cid = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`
    console.error('Save profile error:', { error, cid })
    try { logger.record({ type: 'error', action: 'profile_save', detail: `Save profile error: ${error instanceof Error ? error.message : String(error)} (cid=${cid})` }) } catch {}
    return NextResponse.json(
