...
export default function AdminPage() {
  ...
  const getExpectedAmount = (uniqueCode?: string | null) => {
    const digits = (uniqueCode || '').replace(/\D/g, '')
  ...
  const scanDuplicates = async () => {
    try {
      const res = await fetch('/api/admin/users?duplicates=true')
      const data = await res.json()
      if (!res.ok) {
        alert(data.error || 'Gagal memindai duplikat')
        return
      }
      setDupGroups(data.duplicates || [])
      setShowDupDialog(true)
    } catch (e: any) {
      alert(e.message || 'Error memindai duplikat')
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              title="Buat 30 sample user lengkap (profil + psikotes)"
            >
              {seedLoading ? 'Seeding…' : 'Seed Data (30)'}
            </Button>
            <Button
              variant="outline"
              onClick={async () => {
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Generate sample data untuk pengujian</CardDescription>
              </CardHeader>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      setSeedInfo(null)
                      try {
                        const res = await fetch('/api/admin/seed-sample', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ total: 50 }) })
                        const json = await res.json()
                        if (res.ok) {
                          setSeedInfo(json)
                        } else {
                          setSeedInfo({ error: json.error || 'Gagal membuat sample data' })
                        }
                      } catch {
                        setSeedInfo({ error: 'Terjadi kesalahan jaringan' })
                      } finally {
                        setSeedLoading(false)
                        await loadInsights()
                      }
                  ...
                  >
                    {seedLoading ? 'Memproses...' : 'Generate 50 Sample Users'}
                  </Button>
                </div>
                {seedInfo && (
                  <div className="text-sm">
                    {'error' in seedInfo ? (
                      <span className="text-red-600">{seedInfo.error}</span>
                    ) : (
                      <div className="flex items-center gap-4 flex-wrap">
                        <span className="text-gray-700">Users: <strong>{seedInfo.users}</strong></span>
                        <span className="text-gray-700">Matches: <strong className="text-rose-600">{seedInfo.matches}</strong></span>
                        <span className="text-gray-700">Subscriptions: <strong className="text-green-600">{seedInfo.subscriptions}</strong></span>
                        <span className="text-gray-700">Payments: <strong className="text-blue-600">{seedInfo.payments}</strong></span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" onClick={scanDuplicates}>
                        Scan Duplikat
                      </Button>
                      <Button
                        variant="ghost"
                  ...
                  <TableBody>
                    {users.map((user) => (
                      <TableRow key={user.id}>
                        ...
                        <p className="text-xs text-gray-500">{getInitials(user.profile?.fullName || user.name) || ''}</p>
                          </div>
                        </TableCell>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    {(() => {
                      const exp = getExpectedAmount(selectedPayment.uniqueCode)
                      return (
                        <p className="font-medium">
                          {exp !== null ? `Rp ${exp.toLocaleString('id-ID')}` : '-'}
                        </p>
                      )
                    })()}
                  </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                ...
                (() => {
                  const exp = getExpectedAmount(selectedPayment.uniqueCode)
                  return exp === null || selectedPayment.amount !== exp
                })()
              }
            >
      ...
      <Dialog open={showDupDialog} onOpenChange={setShowDupDialog}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Duplikat Email</DialogTitle>
            <DialogDescription>Email yang memiliki lebih dari satu user</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 max-h-[60vh] overflow-auto">
            {dupGroups.length === 0 ? (
              <div className="text-sm text-gray-600">Tidak ada duplikat</div>
            ) : (
              dupGroups.map((g: any) => (
                <Card key={g.email}>
                  <CardHeader>
                    <CardTitle className="text-sm">{g.email}</CardTitle>
                    <CardDescription>{g.users.length} akun</CardDescription>
                  </CardHeader>
                    ...
                    {g.users.map((u: any) => (
                      <div key={u.id} className="flex items-center justify-between text-sm border rounded p-2">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-gray-500">Bergabung: {new Date(u.createdAt).toLocaleString('id-ID')}</div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge variant="outline">{u.uniqueCode || '-'}</Badge>
                          <Button
                            variant="destructive"
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDupDialog(false)}>Tutup</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
